...
export default {
   ...
   methods: {
     contentScroll(position){//判断profile-nav是否吸顶
        ...
        //下面是判断navbar随滚动区域变化的函数
        const positionY = -position.y 
        let length = this.scrollTop.length
        for(let i=0;i<length-1;i++){
            if(this.currentIndex !==i && (positionY >= this.scrollTop[i] && positionY <= this.scrollTop[i+1])){
                this.currentIndex = i
                this.$refs.nav1.currentIndex = this.currentIndex
                this.$refs.nav2.currentIndex = this.currentIndex
            }
        }
     },
     titleClick(index){
     ...
     selectlist(list){
      ...
      getscrollY(){
        ...
        //加入一个最大的值，这样上面的滚动判断就可以都在两个数之间，而避免各种复杂判断影响性能，max值虽然耗空间但这可以空间换时间
